...
class ConsultaUI():
    ...
    def __init__(self):
        ...
        self.regresarBtton = Button(
            self.raiz, text="Regresar", command=self.listenerRegresar, image=imgButton, relief=GROOVE).place(x=2, y=0)
        ...
        self.tree = ttk.Treeview(
            columns=lb_header, show="headings", selectmode='browse')
        self.scroll = Scrollbar(
            self.raiz, orient="vertical", command=self.tree.yview)
        ...
        self.mostrarBtton = Button(
            self.frame1, text="Mostrar receta", command=self.listennerMostrar).pack()
        self.raiz.mainloop()
    ...
    def listennerMostrar(self):
        ...
        if(not self.tree.focus() == ""):
            ...
            rutaReceta = "baseDatosRecetas/" + \
                dic['values'][0].replace(" ", "") + ".txt"
            ...
            messagebox.showinfo(
                "information", "Debes de seleccionar una receta")
